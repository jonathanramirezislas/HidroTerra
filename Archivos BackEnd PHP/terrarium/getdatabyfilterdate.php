...
$stmt = $pdo->prepare("SELECT * FROM sensor_record WHERE idterrarium=1 AND date BETWEEN :firstDate AND :secondDate ORDER BY date DESC");

          $stmt->bindParam(':firstDate',$_POST['firstDate'] );
              $stmt->bindParam(':secondDate',$_POST['secondDate'] );
            $stmt->execute();
